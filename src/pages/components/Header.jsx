...
function Header() {
    ...
    const handleDetail = (id) => {
        navigate(`/detail/${id}`)
        setMovieData([])
        setCurrentData(prev => ({ ...prev, search: "" }))
    }
    ...
    const getGenre = async (title) => {
        try {
            const data = await getMovieGenre()
            setMovieGenre(data?.data?.genres)
        }
        catch (error) { alert(error) }
    }
        ...
        <div className='nav-container bg-white-trans w-full z-40 absolute top-0'>
            <div className='flex flex-wrap h-full justify-center items-center '>
                <img
                    ...
                    height={31}
                    alt='logo'
                    src={'/asset/moovietime-logo@2x.png'}
                />
                <form className='relative flex px-3 gap-2 rounded mx-9'>
                    ...
                    <input
                        ...
                        value={setCurrentData?.search}
                    />
                    <div className='flex items-center'>
                        <img width={12} height={12} alt='logo' src={'/asset/search-icon@2x.png'} />
                    </div>
                    {movieData.length > 0 &&
                        <div className='absolute w-full p-5 text-white -ml-3 mt-10 bg-black opacity-90 rounded-lg'>
                            {movieData.map(el =>
                                <div
